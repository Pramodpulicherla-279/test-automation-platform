...
export default function AdvancedApiTester() {
  ...
  const sendRequest = async () => {
    try {
      ...
      const resSize = new Blob([JSON.stringify(res.data)]).size;
  ...
  return (
    <div className="api-root">
          ...
          <>
            {params.map((p, i) => (
              <div key={i} className="row">
                ...
                />
                <button onClick={() => removeRow(setParams, params, i)}>❌</button>
              </div>
            ))}
          ...
          <>
            {headers.map((h, i) => (
              <div key={i} className="row">
                ...
                />
                <button onClick={() => removeRow(setHeaders, headers, i)}>❌</button>
              </div>
            ))}
      ...
      <div className="response-meta">
        ...
        {size && <span>Size: {size} bytes</span>}
      </div>

      {/* VIEW SWITCH */}
      <div className="view-switch">
        <button onClick={() => setView("pretty")} className={view === "pretty" ? "active" : ""}>Pretty</button>
        <button onClick={() => setView("raw")} className={view === "raw" ? "active" : ""}>Raw</button>
      </div>
