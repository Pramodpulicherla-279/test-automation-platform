...
if os.path.isdir(merged):
    ...
    if os.path.isdir(history_path):
        ...
        shutil.copytree(history_path, temp_history, dirs_exist_ok=True)
    shutil.rmtree(merged, ignore_errors=True)

os.makedirs(merged, exist_ok=True)

# Restore history
if os.path.exists(os.path.join(project_root, "temp_history")):
    shutil.copytree(
        os.path.join(project_root, "temp_history"),
        os.path.join(merged, "history"),
        dirs_exist_ok=True
    )
    os.makedirs(merged, exist_ok=True)
...
def _merge_device_results(project_root: str, current_servers: list) -> None:
    """
    After all workers finish, copy every per-device result file into the
    shared RESULTS_DIR.  Files from different devices never collide because
    allure generates UUID-named JSON files.

    Also injects a `device` executor label into every result file so the
    Allure UI can filter/group by device.
    """
    ...
    for srv in current_servers:
        device = srv.get("device", "unknown")
        src_dir = os.path.join(device_root, device)
        ...
        # ── Inject device label into every *-result.json ──────────────────
        for rf in result_files:
            try:
                with open(rf, "r", encoding="utf-8") as fh:
                    data = _json.load(fh)

                # Add / overwrite the device label so Allure can filter by it
                labels = data.get("labels", [])
                # Remove any existing device labels
                labels = [l for l in labels if l.get("name") != "device"]
                labels.append({"name": "device", "value": device})

                # Also add Android OS version if available (set by conftest)
                # Preserve existing tag labels (they carry OS info)
                data["labels"] = labels
                ...
                shutil.copy2(rf, merged_dir)
                total_copied += 1

        # ── Copy attachments (screenshots, logs, etc.) ────────────────────
        for af in attachment_files:
            try:
                dest = os.path.join(merged_dir, os.path.basename(af))
                # Avoid name collisions by prefixing with device id
                if os.path.exists(dest):
                    base, ext = os.path.splitext(os.path.basename(af))
                    dest = os.path.join(merged_dir, f"{device}_{base}{ext}")
                shutil.copy2(af, dest)
            except Exception as e:
                print(f"[MERGE] Attachment copy failed {af}: {e}")

    print(f"[MERGE] Copied {total_copied} result file(s) from {len(current_servers)} device(s) → {merged_dir}")
...
def run_tests_and_get_suggestions(
    ...
    pytest_args = valid_paths + [
        f"--apk={apk_path}",
        "-v",
        # Pass the per-device results root so conftest can build the path
