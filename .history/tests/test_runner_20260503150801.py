...
def generate_report(project_root: Optional[str] = None) -> None:
    ...
    # ── 1. Results already in RESULTS_DIR ────────────────────────────────
    # FIX: All xdist workers write directly to --alluredir (RESULTS_DIR)
    # using allure-pytest's built-in UUID-named files.  The per-device
    # merge step (_merge_device_results) was reading from empty per-device
    # subdirs (because conftest._get_worker_alluredir was never called),
    # producing 0 merged files → Allure showed 0 test cases.
    # Device grouping is handled by allure.dynamic.parent_suite() in the
    # driver fixture, not by filesystem separation.
    current_servers = get_servers()
    send_log(
        f"[Report] Collecting results from {len(current_servers)} device(s) in {RESULTS_DIR}",
        "INFO"
    )
...
def run_pytest_streaming_with_tracking(
    ...
    # FIX: Do NOT pass --clean-alluredir to pytest.
    # _ensure_clean_allure_dirs() already wiped RESULTS_DIR before this
    # subprocess starts. If we pass --clean-alluredir again, pytest wipes
    # the dir a SECOND time at collection start — after environment.properties
    # was written and before any test result is saved. This is why Allure
    # showed 0 test cases: the dir was empty when allure generate ran.
    cmd += pytest_args
