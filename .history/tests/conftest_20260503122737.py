...
def _get_worker_alluredir(config, device_name: str) -> Optional[str]:
    ...
    # 🔥 MAKE DEVICE DIR UNIQUE PER WORKER
    worker_id = os.environ.get("PYTEST_XDIST_WORKER", "gw0")
    device_dir = f"{device_name}_{worker_id}"

    d = os.path.join(root, device_dir)
...
def pytest_runtest_makereport(item, call):
    ...
    # 🔥 MAKE TEST UNIQUE PER DEVICE
    unique_test_name = f"{item.name} [{dev_name}]"
    allure.dynamic.title(f"{item.name} [{dev_name}]")
    
    allure.dynamic.title(unique_test_name)
    
    # Device grouping (accordion)
