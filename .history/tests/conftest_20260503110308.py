...
def handle_permissions(driver):
    print("🔐 Handling permissions (robust)...")

    for _ in range(5):  # retry multiple popups
        try:
            ...
            print("⚠️ Permission check skipped:", e)
...
def _get_worker_alluredir(config, device_name: str) -> Optional[str]:
    """
    Return the per-device allure results directory for the current worker,
    or None if --device-results-root was not passed (single-device run).
    """
    root = config.getoption("--device-results-root", default=None)
    if not root:
        root = os.environ.get("DEVICE_RESULTS_ROOT")
    if not root or not device_name:
        return None
    d = os.path.join(root, device_name)
...
def device_id(request):
    ...
    # ── Redirect this worker's allure output to per-device subdir ─────────
    per_device_dir = _get_worker_alluredir(request.config, dev_id)
    if per_device_dir:
        # Override the alluredir that allure_pytest uses for this process
        try:
            # allure-pytest stores its listener on the plugin manager
            listener = request.config.pluginmanager.get_plugin("allure_listener")
            if listener and hasattr(listener, "allure_logger"):
                logger_obj = listener.allure_logger
                if hasattr(logger_obj, "_dest"):       # allure-python-commons ≥ 2.x
                    logger_obj._dest = per_device_dir
                elif hasattr(logger_obj, "results_dir"):  # older API
                    logger_obj.results_dir = per_device_dir
            print(f"[ALLURE] Worker {worker_id} → writing results to {per_device_dir}")
        except Exception as e:
            print(f"[ALLURE] Could not redirect alluredir for {worker_id}: {e}")
    ...
    allure.dynamic.label("worker",  worker_id)    allure.dynamic.label("os",      os_version)
...
def driver(request):
    ...
    time.sleep(3)  # 🔥 important stabilization
...
def pytest_runtest_makereport(item, call):
    ...
    allure.dynamic.tag(os_version)
    # 🔥 THIS CREATES DEVICE ACCORDION IN UI
    allure.dynamic.parent_suite(dev_name)
    
    # 🔥 THIS KEEPS MODULE STRUCTURE INSIDE DEVICE
