...
async def jira_create_flow(req):
    ...
    try:
        with _ctx.redirect_stdout(_captured):
            result = create_and_store_jira_issue(
                ...
                worker_id=getattr(req, "worker_id",  None),
    ...
    entry = {
        ...
        "created_at": datetime.datetime.now().isoformat(),
        # Device traceability — passed from conftest payload via JiraCreateRequest
        "device_id":  getattr(req, "device_id",  None) or "",
        "worker_id":  getattr(req, "worker_id",  None) or "",
        "os_version": getattr(req, "os_version", None) or "",
    }
...
def _build_device_groups(issues: list) -> dict:
    """
    Group issues by device_id for the Issue Panel accordion.

    Returns:
        {
            "emulator-5554": [issue, issue, ...],
            "emulator-5556": [issue, ...],
            "unknown":       [issue, ...]   # issues without device_id
        }
    """
    groups: dict = {}
    for issue in issues:
        dev = issue.get("device_id") or "unknown"
        groups.setdefault(dev, []).append(issue)
    return groups
...
async def jira_history_api_flow(
    ...
    def _normalise(t: dict) -> dict:
        ...
        return {
            ...
            # Device traceability — enables device-wise grouping in Issue Panel
    ...
    if is_mongodb_enabled():
        try:
            ...
            # Build per-device grouped structure for Issue Panel accordion
            device_groups = _build_device_groups(normalised)
            return {
                "issues":        normalised,
                "device_groups": device_groups,   # NEW: for device-wise accordion
                "source":        "mongodb",
    ...
    device_groups = _build_device_groups(memory_issues)
    return {
