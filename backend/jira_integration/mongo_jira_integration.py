...
FIXES (v3):
  • create_and_store_jira_issue() now passes ticket_type="created", sprint,
    fix_version, affects_version, and issue_url to save_ticket() so MongoDB
    documents are complete and match the frontend schema.
  • store_removed_ticket() now explicitly passes ticket_type="removed"
    so dismissed tickets appear in the Unassigned tab correctly.
  • Both functions pass all available metadata to avoid "Unknown" defaults.
"""
