...
FIXES (v3):
  • get_all_tickets() now re-raises exceptions instead of swallowing them.
    This allows jira_history_api to fall back to session memory when MongoDB
    is unavailable, instead of silently returning 0 results.
  • save_ticket() no longer defaults test_id to "Unknown" — stores "" or actual value.
  • save_ticket() no longer defaults start_date to datetime.now() — stores None if absent.
  • ticket_type field ("created" | "removed") is always stored.
  • All version/sprint/url fields persisted correctly.
"""
...
def _generate_ticket_id() -> str:
    """Generate a unique internal ticket ID."""
