...
def deploy_to_vercel() -> str | None:
    """Deploy allure-report folder to Vercel and return live URL."""
    ...
    print("[Vercel] Deploying allure-report to Vercel...")
...
def _run_post_notify(**kwargs) -> None:
    """Sync wrapper so FastAPI BackgroundTasks can schedule the async _post_run_notify."""
...
async def _post_run_notify(
    ...
    """
    Unified post-run handler used by BOTH manual UI runs AND Slack-triggered runs.
    Flow: run tests → generate Allure report → count results → send Slack notification.
    """
    ...
    print(f"[PostRun] Step 5: Sending Slack notification to channel {final_channel_id}...")
    ...
    try:
        ...
        await manager.broadcast({
            "type": "LOG",
            "payload": {
                "message": f"⚠️ Slack notification failed: {e}",
                "status": "WARN"
            }
        })
...
async def _handle_slack_apk(
    ...
    """
    Runs entirely as a background task.
    Downloads APK → resolves metadata → starts Appium → runs tests → deploys → sends Slack link.
    """
...
async def get_app_variants():
    """
    Returns the canonical test suite definitions for every app variant.
    The frontend must fetch from here — never hardcode paths on the frontend.
    """
    return {
...
async def enhance_jira_issue(req: JiraEnhanceRequest):
    ...
    and returns them so the frontend can paste directly into the issue card fields.
    """
