...
function TestScreen() {
    ...
    const [jiraIssues, setJiraIssues] = useState(() => loadState('jiraIssues', []));
    ...
    const [uiAnalysisStatus, setUiAnalysisStatus] = useState('idle'); // 'idle' | 'loading' | 'ready' | 'error'
    ...
    const analyzeUiScreenshots = async () => {
        try {
            setUiAnalysisStatus('loading');
            setUiAnalysisError('');

            const res = await fetch(`${API_URL}/api/ui-screenshots/analyze`, {
                method: 'POST',
            ...
            setUiAnalysisResults(data.results || []);
            setUiAnalysisStatus('ready');
        } catch (e) {
            setUiAnalysisStatus('error');
            setUiAnalysisError(e?.message || 'Unknown error');
        }
    };
    ...
    useEffect(() => {
        ...
        // Persist Jira payloads (limit to last 200)
        sessionStorage.setItem('jiraIssues', JSON.stringify((jiraIssues || []).slice(-200)));
    }, [apkUrl, isRunning, selectedAppKey, modules, selectedApk, logs, jiraIssues]);
                ...
                try {
                    ...
                    setLogs(prev => [
                        ...prev,
                        {
                            ...
                            message: `Failed to parse JIRA payload JSON: ${e?.message || 'Unknown error'}`,
                            type: 'FAILED'
                        }
                    ]);
                }
    ...
    const handleRunTest = async () => {
        ...
        try {
            ...
            if (!response.ok) {
                // Surface the real error from backend before throwing
                const detail = data?.detail || 'Failed to start test';
                handleIncomingData({
                    type: 'LOG',
                    payload: { message: `❌ Server error: ${detail}`, status: 'FAILED' }
                });
    ...
    return (
        <div>
            <Header
                ...
                uiIssuesOpen={showUiIssuesScreen}
                uiIssuesLoading={uiAnalysisStatus === "loading"}
                onToggleUiIssues={() => {
                    setShowUiIssuesScreen((prev) => !prev);
                }}
            />
            {showUiIssuesScreen && (
                <div className="ui-issues-overlay" role="dialog" aria-modal="true">
                    <div className="ui-issues-overlay-inner">
                        <UIScreenshotIssues
                            ...
                            error={uiAnalysisError}
                            results={uiAnalysisResults}
                            onAnalyzeClick={analyzeUiScreenshots}
            ...
            <div className="dashboard-grid">
                ...
                <div className="grid-item-logs" style={{ display: 'flex', gap: '1rem', alignItems: 'stretch' }}>
                    ...
                    <div style={{ flex: '0 0 calc(40% - 1rem)', minWidth: 0 }}>
                        <IssuePanel modules={modules} jiraIssues={jiraIssues} apiUrl={API_URL} />
                    </div>
                </div>
            </div>
        </div>
