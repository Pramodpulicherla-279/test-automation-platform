...
class UserOnboardingPage:
    ...
    def click_organization(self):
        with allure.step("Click Organization"):
             self.page.locator(self.locators["Organization"]).wait_for(state="visible", timeout=10_000)
    ...
    def click_user_role_option(self):
        with allure.step("Select User Role option"):
            self.page.wait_for_selector(self.locators["add_user"]["user_role_option"], state="visible")
            self.page.wait_for_timeout(2000)
            self.page.click(self.locators["add_user"]["user_role_option"])
